...
SimpleWindow::SimpleWindow(ColorSqureMatrix *canvas, ColorSqureMatrix *paint)  :
        window(sf::RenderWindow(
                sf::VideoMode(
                    (SQUARE_SIZE + PAINT_CAN_SIZE) * (side_size + margin_size) + 50 + margin_size * 2,
                    (SQUARE_SIZE > PAINT_CAN_SIZE ? SQUARE_SIZE : PAINT_CAN_SIZE) * (side_size + margin_size) + 30 + margin_size * 2),
...
void SimpleWindow::paintMatrix(std::vector<RectangleShape> &rec_matrix, ColorSqureMatrix *color_matrix) {
    for(unsigned i = 0; i < color_matrix->CellCount(); i++) {
        auto rec = &rec_matrix[i];
        unsigned col, row;
        row = i / color_matrix->Size();
